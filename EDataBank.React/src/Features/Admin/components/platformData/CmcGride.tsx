...
import { ICmc} from "../../types/interface";
import AddCmcDialog from "./dialog/AddCmcDialog";
//   import MemberRegistrationDialog from "./dialog/MemberRegistrationDialog";
  ...
  const CmcGride = observer((props: Props) => {
  ...
  
                {
                    ...
                    onClick: (
                      ...
                      item?: IContextualMenuItem | undefined
                        ...
                        adminStore.editMode = false;
                        PlatformStore.cmc = {} as ICmc;
                        adminStore.showCmcAddDialog = true;
                    },
                  },
                  {
                    key: "edit",
                  ...
                  },
                  {
                    ...
                    onClick: () => {
                      let cmc = PlatformStore.cmc;
                      confirm({
                        title: `Confirm  ${cmc.cmcId}  cmc`,
                        content: `${cmc.cmcId} cmc will be remove from this list`,
                        okText: "Yes",
                        okType: "danger",
                        cancelText: "No",
                        onOk: async () => {
                          try {
                            await PlatformStore.removeCmc(cmc.cmcId)
                            notification.success({
                              message: "EDataBank Platform Feedback",
                              description: `${cmc.cmcOrder} removed successfully`,
                            });
                          } catch (error) {
                            notification.info({
                              message: "EDataBank Platform Feedback",
        ...
        <div className="ms-Grid-row" style={{ paddingTop: "10px" }}>
          {PlatformStore.isLoading  ? (
            ...
            <div className="ms-Grid-col ms-sm12">
              ...
              >
                <AgGridReact
                  rowData={[...PlatformStore.cmcs.values()].map((row) => toJS(row))}
                  ref={grid}
                  gridOptions={{
                    defaultColDef: {
                      sortable: true,
