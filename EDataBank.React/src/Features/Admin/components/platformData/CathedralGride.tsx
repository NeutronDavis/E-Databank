...
import { ICathedral } from "../../types/interface";
...
//   import MemberRegistrationDialog from "./dialog/MemberRegistrationDialog";
  ...
  const CathedralGride = observer((props: Props) => {
    ...
    const rowSelected = (grid: any) => {
      ...
      if (selrow) {
        PlatformStore.cathedral = selrow.data
      ...
      columnDefs: [
        ...
        },
        {
          ...
          cellRendererFramework: function (props: any) {
            const { priorityId } = props.data;
            return PlatformStore.prioritys.get(priorityId)?.priorityName
          },
        },
      ],
    };
      ...
      <div className="ms-Grid" dir="ltr">
        <div className="ms-Grid-row">
          <div className="ms-Grid-col ms-sm12 ms-md8 ms-lg8">
            <SubPageHeaderTitle
              count={PlatformStore.cathedrals.size}
              title={"Cathedrals "}
            />
          </div>
          {/* <div
  ...
  
                {
                    ...
                    onClick: (
                      ...
                      item?: IContextualMenuItem | undefined
                        ...
                        adminStore.editMode = false;
                        PlatformStore.cathedral = {} as ICathedral
                        adminStore.showCathedralAddDialog = true;
                    },
                  },
                  {
                    key: "edit",
                    ...
                    disabled: _.isUndefined(
                        PlatformStore.cathedral.cathedralId
                    ),
                    onClick: () => {
                        adminStore.showCathedralAddDialog = true;
                    },
                  ...
                  {
                    key: "delete",
                    text: "Remove Cathedral",
                    onRenderIcon: () => <Trash2 className="qicon" />,
                    disabled: _.isUndefined(
                      PlatformStore.cathedral.cathedralId
                    ),
                    onClick: () => {
                      let cathedral = PlatformStore.cathedral;
                      confirm({
                        title: `Confirm  ${cathedral.cathedralName}  cathedral`,
                        content: `${cathedral.cathedralName} cathedral will be remove from this list`,
                        okText: "Yes",
                        okType: "danger",
                        cancelText: "No",
                        onOk: async () => {
                          try {
                            await PlatformStore.removeCathedral(cathedral.cathedralId)
                            notification.success({
                              message: "EDataBank Platform Feedback",
                              description: `${cathedral.cathedralName} removed successfully`,
                            });
                          } catch (error) {
                            notification.info({
                              message: "EDataBank Platform Feedback",
        ...
        <div className="ms-Grid-row" style={{ paddingTop: "10px" }}>
          {PlatformStore.isLoading ? (
            ...
            <div className="ms-Grid-col ms-sm12">
              ...
              >
                <AgGridReact
                  rowData={[...PlatformStore.cathedrals.values()].map((row) => toJS(row))}
                  ref={grid}
                  gridOptions={{
                    defaultColDef: {
                      sortable: true,
